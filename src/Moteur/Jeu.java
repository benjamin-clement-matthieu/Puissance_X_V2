...
public class Jeu {
	...
	public boolean jouerCoup(int col, Joueur joueur)
	{
		if (col <= tVerticale &&  col >= 0)
		{
			...
			// Si le coup peut Ãªtre jouÃ©
			if (ligneChoisie != -1)
			{
				// On parcout chaque case jusqu'Ã  la ligne choisie
				for (int i = 0; i < ligneChoisie; i++)
				{
					...
					// On remet la couleur de la case Ã  VIDE
	...
	public boolean verifierGagnant(Joueur joueur)
	{
		for (int i = 0; i < tHorizontale; i++)
			for (int j = 0; j < tVerticale; j++)
				//Si on a trouvÃ© un jeton du joueur, on regarde dans toutes les directions
	...
	public ArrayList<Integer> getCoupsPossible()
	{
		...
		// On regarde chaque case de la premiÃ¨re ligne, si une est vide la grille le coup est possible.
		for (int i = 0; i < tHorizontale; i++)
